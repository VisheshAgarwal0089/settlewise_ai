...
export function resetAllData(database, { confirmation, userId, requestId }) {
  ...
  return database.transaction(() => {
    const deletedWithInternalCounts = {
    ...
    const finalEvent = appendAuditEvent(database, { userId, eventType: 'RESET', entityType: 'system', requestId, payload: { deleted: deletedWithInternalCounts } });
    console.info(JSON.stringify({ event: 'RESET', requestId, userId, eventHash: finalEvent.eventHash, deleted: deletedWithInternalCounts }));
    database.prepare('DELETE FROM audit_logs').run();
    database.prepare('DELETE FROM batches').run();
    const deleted = {
      batches: deletedWithInternalCounts.batches, orders: deletedWithInternalCounts.orders,
      settlementRecords: deletedWithInternalCounts.settlementRecords, matches: deletedWithInternalCounts.matches,
      reviewActions: deletedWithInternalCounts.reviewActions, explanations: deletedWithInternalCounts.explanations,
      auditLogs: deletedWithInternalCounts.auditLogs, dataImports: deletedWithInternalCounts.dataImports
    };
