...
export function getMatchDetail(database, matchId) {
  ...
  const settlementColumns = 'id,entity_id,order_id,order_receipt,payment_id,settlement_id,settlement_utr,amount_paise,fee_paise,tax_paise,credit_paise,settled_at_utc';
  const settlements = candidates.length ? database.prepare(`SELECT ${settlementColumns} FROM settlement_records WHERE id IN (${candidates.map(() => '?').join(',')})`).all(...candidates.map((candidate) => candidate.settlementRecordId)) : [];
  const selectedSettlement = row.settlement_record_id ? database.prepare(`SELECT ${settlementColumns},batch_id FROM settlement_records WHERE id=?`).get(row.settlement_record_id) : null;
  const serializeSettlement = (item) => item ? ({ id: item.id, entityId: item.entity_id, orderId: item.order_id, orderReceipt: item.order_receipt, paymentId: item.payment_id, settlementId: item.settlement_id, settlementUtr: item.settlement_utr, amountPaise: item.amount_paise, feePaise: item.fee_paise, taxPaise: item.tax_paise, creditPaise: item.credit_paise, settledAtUtc: item.settled_at_utc }) : null;
  const selectedRefunds = selectedSettlement ? findRefundsForPayment(database, selectedSettlement) : [];
  return { ...serializeMatch(database, matchId), order: { id: order.id, merchantOrderId: order.merchant_order_id, orderReceipt: order.order_receipt, grossAmountPaise: order.gross_amount_paise, refundAmountPaise: order.refund_amount_paise, expectedFeePaise: order.expected_fee_paise, expectedTaxPaise: order.expected_tax_paise, expectedNetPaise: order.expected_net_paise, orderDateUtc: order.order_date_utc, generationCase: order.generation_case }, settlement: selectedSettlement ? { ...serializeSettlement(selectedSettlement), refundAmountPaise: selectedRefunds.reduce((sum, refund) => sum + refund.debit_paise, 0) } : null, deterministicEvidence: JSON.parse(row.evidence_json), candidateSettlements: settlements.map(serializeSettlement), aiExplanation: explanation ? { status: explanation.status, model: explanation.model, category: explanation.category, summary: explanation.summary, recommendedAction: explanation.recommended_action, errorCode: explanation.error_code, createdAt: explanation.created_at, completedAt: explanation.completed_at } : null };
}
